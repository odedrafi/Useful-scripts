...
aws s3 cp s3://$env:BUCKET_INPUT_NAME/input $Local_input_path --recursive
...
Copy-Item -Path $Local_in_Progres_path -Destination $Local_output_path -recurse -Force
...
aws s3 cp $Local_output_path s3://$env:BUCKET_INPUT_NAME/$env:ALGORITHM_NAME/output
